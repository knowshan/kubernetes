...
func TestRecycleSync(t *testing.T) {
	...
	tests := []controllerTest{
		...
		},
		{
			...
			initialClaims:   noclaims,
			expectedClaims:  noclaims,
			expectedEvents:  []string{"Warning VolumeFailedRecycle"}, errors: noerrors, test: testSyncVolume,
		},
		{
			...
			initialClaims:   noclaims,
			expectedClaims:  noclaims,
			expectedEvents:  []string{"Warning VolumeFailedRecycle"}, errors: noerrors,
		...
		},
		{
			...
			initialClaims:   noclaims,
			expectedClaims:  noclaims,
			expectedEvents:  []string{"Warning VolumeUnknownReclaimPolicy"}, errors: noerrors, test: testSyncVolume,
		},
		{
			...
			initialClaims:   noclaims,
			expectedClaims:  noclaims,
			expectedEvents:  []string{"Normal VolumeFailedRecycle"}, errors: noerrors, test: testSyncVolume,
		},
		{
	...
	tests := []controllerTest{
		{
			...
			// try again.
			name:            "7-1 - recycle returns error",
			initialVolumes:  newVolumeArray("volume7-1", "1Gi", "uid7-1", "claim7-1", v1.VolumeBound, v1.PersistentVolumeReclaimRecycle, classEmpty),
			expectedVolumes: newVolumeArray("volume7-1", "1Gi", "", "claim7-1", v1.VolumeAvailable, v1.PersistentVolumeReclaimRecycle, classEmpty),
			initialClaims:   noclaims,
			expectedClaims:  noclaims,
			expectedEvents:  []string{"Warning VolumeFailedRecycle"}, errors: noerrors,
			test: wrapTestWithReclaimCalls(operationRecycle, []error{errors.New("Mock recycle error"), nil}, testSyncVolume),
		},
	}
